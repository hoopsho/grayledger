...
## Decision
Any user whose email domain is `@grayledger.io` (or is in a hard-coded allow-list) automatically becomes a platform superuser.
...
def self.platform_staff?
  user
  user&.email&.end_with?("@grayledger.io") || %w[you@youremail.com].include?(user.email)
end
...
- All actions appear in customer's activity log as "Platform Support (you@grayledger.io) fixed X"
...
This is the parachute that is the difference between a good app and a bulletproof business.
