...
Rubyclass TransactionPolicy < ApplicationPolicy
  def update?
...
Ruby# in every policy
def owner_or_platform_staff?
  current_role == "owner" || Current.platform_staff?
end
Current.platform_staff? is true only when the logged-in user.email ends with @ledgerai.com (or is in a hard-coded list for now).
Consequences
