...
Generate embedding from merchant + description + amount bucket
Query pgvector for 10 nearest neighbors in the same company
If best match cosine similarity ≥ 0.92 and same amount (±1 %) → auto-post using the exact same accounts as before (confidence = 99.9 %)
Only if no good vector match → send to full GPT-4o prompt

Embedding generation (once per decision)
Ruby# app/services/transaction_embedder.rb
